...
import BottomPart from '../../components/OrderCart/BottomPart'
import { Minus, Plus, ShoppingBag, Trash2 } from 'lucide-react'
import { calculateItemTotal, calculateTotal } from '../../utils/calculation'
import DeliveryOptions from '../../components/OrderCart/DeliveryOptions'

const Checkout = () => {
    const location = useLocation()
    const orderItems = location.state

    const [orderCart, setOrderCart] = useState<OrderProps[]>(orderItems)

    console.log("orderItems in Checkout is: ", orderCart)
  ...
  const handleRemoveItem = (itemId: number) => {
    ...
    // Delivery options: Dine in, Pick Up, Delivery 

    return (
        <div className="bg-white border-2 border-gray-200 rounded-3xl shadow-xl p-6 min-h-screen flex flex-col ">
            {/* Header */}
                ...
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p className="text-sm text-gray-600 font-semibold">
                            {orderCart.length} {orderCart.length === 1 ? 'item' : 'items'}
                        </p>
                    </div>
                </div>
            </div>
            ...
            <div className="max-h-80 overflow-y-auto mb-6 space-y-4 scrollbar-custom pr-2">
                {orderCart.length === 0 ? (
